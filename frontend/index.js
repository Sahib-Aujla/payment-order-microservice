    ...
    fetch(URL_API)
        ...
        .then(data => {
            ...
            data.forEach(order => {
                ...
                row.insertCell(3).innerText = new Date(order.orderDate).toLocaleString();
                ...
                actionsCell.appendChild(createButton('Delete', () => deleteOrder(order.id)));
                actionsCell.appendChild(createButton('Update', () => updateOrderForm(order)));
            });
        });
}
    ...
    fetch(URL_PAY)
        ...
        .then(data => {
            ...
            data.forEach(payment => {
                ...
                row.insertCell(3).innerText = new Date(payment?.paymentDate).toLocaleString();
...
function deleteOrder(orderId) {
    ...
    fetch(URL_API+`/delete/${orderId}`, { method: 'DELETE' })
        .then(() => fetchOrders()); 
}
